...
export default function Sidebar({
  ...
  const noteLink = (note) => (
    <div
      key={note._id}
      className={`w-full flex items-center gap-2 px-3 rounded-lg transition-all ${selectedNoteId === note._id ? "bg-secondary-container text-on-secondary-container font-semibold" : "text-on-surface-variant hover:bg-surface-container-high"}`}
    >
      <button
        type="button"
      ...
      >
        <span className="material-symbols-outlined text-[20px]!">
          {isBin ? "delete" : note.isPinned ? "push_pin" : "description"}
        </span>
      ...
      {isBin ? (
        <>
          ...
          <button
            type="button"
            onClick={() => onPermanentlyDeleteNote(note)}
        ...
        <button
          ...
          className="shrink-0 text-on-surface-variant hover:text-error"
          title="Move to Bin"
          aria-label={`Move ${note.title || "Untitled Note"} to Bin`}
        >
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-72 bg-surface-container-low z-50 flex flex-col border-r border-outline-variant">
      ...
      <div className="flex-1 overflow-y-auto px-inset-md ">
        <label className="flex items-center gap-2 px-3 py-1 rounded-lg bg-surface-container-high text-on-surface-variant">
        ...
        <div className={isBin ? "hidden" : ""}>
          <div className="flex items-center justify-between px-3 mb-stack-sm">
            ...
            <button
              type="button"
              onClick={onNewFolder}
              className="text-on-surface-variant rounded hover:bg-surface-container-high transition-all"
              title="New project"
          ...
          <nav className="space-y-stack-xs">
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-1 rounded-lg text-left ${!selectedFolderId ? "bg-secondary-container text-on-secondary-container font-semibold" : "text-on-surface-variant hover:bg-surface-container-high"}`}
            >
              <span className="material-symbols-outlined text-[20px]">
                apps
              </span>
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-1 rounded-lg text-left ${selectedFolderId === "unfiled" ? "bg-secondary-container text-on-secondary-container font-semibold" : "text-on-surface-variant hover:bg-surface-container-high"}`}
            >
              <span className="material-symbols-outlined text-[20px]">
                folder_open
              </span>
        ...
        <div className={isBin ? "display" : "hidden"}>
          <div className="flex items-center justify-between px-3 mb-stack-sm">
            <p className="font-label-caps text-label-caps text-outline uppercase">
              Deleted Notes
            </p>
            {(binNotes.length > 0 || binFolders.length > 0) && (
              <button
                type="button"
                onClick={onEmptyBin}
                  ...
                  .map((folder) => (
                    <div key={folder._id} className="space-y-stack-xs">
                      <div className="w-full flex items-center gap-2 px-3 rounded-lg text-on-surface-variant">
                        ...
                        <button
                          type="button"
                          onClick={() => onPermanentlyDeleteFolder(folder)}
